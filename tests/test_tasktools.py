...
from dicetables import (DiceTable, Die, ModDie, WeightedDie, ModWeightedDie,
...
class TestTaskTool(TestCase):
    ...
    def test_extract_modifiers_no_modifiers(self):
        ...
        dice = sorted([(die, 3), (weighted, 4), (StrongDie(die, 3), 5), (ExplodingOn(die, (1, )), 6),
    ...
    def test_extract_modifiers_cannot_extract_from_nested_dice(self):
        ...
        dice = sorted([(StrongDie(mod, 5), 10), (ExplodingOn(mod_die, (1,)), 10), (Exploding(mod_weighted), 10)])
        self.assertEqual(extract_modifiers(dice), (0, dice))

    def test_extract_modifiers_on_Modifier(self):
        dice = [(Modifier(5), 3)]
        self.assertEqual(extract_modifiers(dice), (15, []))
    ...
    def test_extract_modifiers_on_ModDie(self):
        dice = [(ModDie(6, 5), 4)]
        self.assertEqual(extract_modifiers(dice), (20, [(Die(6), 4)]))
    ...
    def test_extract_modifiers_on_ModWeightedDie(self):
        ...
        dice = [(ModWeightedDie(die_dict, 6), 5)]
        self.assertEqual(extract_modifiers(dice), (30, [(WeightedDie(die_dict), 5)]))
    ...
    def test_extract_modifiers_many_modifier_dice(self):
        die_dict = {roll: roll % 3 for roll in range(1, 10)}
        dice = sorted([(ModDie(6, -5), 4), (ModWeightedDie(die_dict, 6), 5), (Modifier(-5), 3)])
        new_dice = sorted([(Die(6), 4), (WeightedDie(die_dict), 5)])
        self.assertEqual(extract_modifiers(dice), (-20 + 30 + -15, new_dice))

    def test_extract_modifiers_many_dice(self):
        die_dict = {roll: roll % 3 for roll in range(1, 10)}
        dice = sorted([(ModDie(6, -5), 4), (Die(7), 3), (ModWeightedDie(die_dict, 6), 5), (WeightedDie({1: 2}), 10),
                       (Modifier(-5), 3), (StrongDie(Die(6), 2), 11)])

        dice_no_mods = sorted([(Die(6), 4), (Die(7), 3), (WeightedDie(die_dict), 5),
    ...
    def test_extract_modifiers_combines_new_die_with_same_modless_die(self):
        ...
        dice = sorted([(die, 2), (die_pos, 3), (die_neg, 4),
                       (weighted, 5), (weighted_pos, 6), (weighted_neg, 7),
                       (different_die, 8), (different_weighted, 9)])
        new_dice = sorted([(die, 2 + 3 + 4), (weighted, 5 + 6 + 7), (different_die, 8), (different_weighted, 9)])
